...
if (!class_exists('Envato_Theme_Setup_Wizard')) {
    ...
    class Envato_Theme_Setup_Wizard {
        ...
        public function init_globals() {
            ...
            $this->plugin_url = medicll_framework_get_url('envato_setup');
        }
        ...
        private function _content_rev_slider_install() {
            ...
            if (function_exists('medicll_demo_data_get_path')) {
                $theme_style = medicll_demo_data_get_path('content/' . basename(get_theme_mod('dtbwp_site_style', $this->get_default_theme_style())) . '/');
        ...
        private function _fetch_remote_file($url, $post) {
            ...
            if (function_exists('medicll_demo_data_get_path')) {
                $local_file = medicll_demo_data_get_path('content/images/') . $file_name;
            } else {
                $local_file = medicll_framework_get_path('demo-data-setup/content/images/stock/') . $file_name;
            }
            $upload = false;
        ...
        public function _theme_options_install() {
            ...
            if (isset($custom_options['medicll_framework_options']) && class_exists('ReduxFrameworkInstances')) {
                $optionsss = json_decode($custom_options['medicll_framework_options'], true);
                $redux = ReduxFrameworkInstances::get_instance('medicll_framework_options');
                $redux->set_options($optionsss);
            }
        ...
        public function _get_json($file, $path = '') {
            ...
            if (is_file($theme_style . basename($file))) {
                ...
                if (file_exists($file_name)) {
                    ...
                    if (strpos($file_content, 'https:\\\/\\\/medicll.net\\\/' . basename(get_theme_mod('dtbwp_site_style', $this->get_default_theme_style())) . '\\\/') !== false) {
                        $find_url = 'https:\\\/\\\/medicll.net\\\/' . basename(get_theme_mod('dtbwp_site_style', $this->get_default_theme_style())) . '\\\/';
                        $file_content = str_replace(array($find_url), array($replace_url_with), $file_content);
                    } else if (strpos($file_content, 'https:\\\/\\\/medicll.net\\\/demo\\\/') !== false) {
                        $find_url = 'https:\\\/\\\/medicll.net\\\/demo\\\/';
        ...
        public function envato_setup_logo_design_save() {
            ...
            global $MedicllFrameReduxFramework;
            $theme_options = get_option('medicll_framework_options');
            ...
            if ($new_logo_id) {
                ...
                if ($attr && !empty($attr[1]) && !empty($attr[2])) {
                    ...
                    if ($attr && isset($attr[0]) && !empty($attr[0])) {
                        ...
                        //$MedicllFrameReduxFramework->ReduxFramework->set('medicll-site-logo', $logo_image);
                        $theme_options['medicll-site-logo']['url'] = $attr[0];
                        update_option('medicll_framework_options', $theme_options);
                    }
                }
            } else {
                $image_url = $this->plugin_url . '/images/' . get_theme_mod('dtbwp_site_style', $this->get_default_theme_style()) . '/logo.png';
                ...
                //$MedicllFrameReduxFramework->ReduxFramework->set('medicll-site-logo', $logo_image);
                $theme_options['medicll-site-logo']['url'] = $image_url;
                update_option('medicll_framework_options', $theme_options);
            }
